"""小物体包络原语 — 拇指与四指(食中无小)同时力控闭合，适合小型多面体抓取。

抓取类型: 1 vs 2-5 (拇指 vs 食指+中指+无名指+小指)
别名: polyhedron, strength
...
# --- 物体尺寸 → thumb_rot 映射 ---
GRASP_WIDTH_MIN = 10.0
GRASP_WIDTH_MAX = 50.0
THUMB_ROT_SMALL = 200
THUMB_ROT_LARGE = 120
...
class SmallWarpByVision(HandGesturePrimitive):
    """拇指与四指全包络力控闭合抓取 (1 vs 2-5)。

    五指同时包络，适合小型多面体或需要力量抓取的场景。
    四根手指从正面包围物体，拇指从对侧施压。
    ...
    def _compute_thumb_rot(self, ctx: PrimitiveContext) -> int:
        ...
        if has_real_tcp:
            ...
            width_mm = min(extents)

        rot = _width_to_thumb_rot(width_mm)
        _logger.warning("small_warp: 物体=%s, 宽度=%.1fmm → thumb_rot=%d", label, width_mm, rot)
        return rot
    ...
    def compute(
        ...
        if self._phase == _Phase.PRESHAPE:
            ...
            if t >= 1.0:
                ...
                _logger.warning("small_warp: 进入手指闭合阶段（四指先合）")
        ...
        if self._phase == _Phase.CLOSE_FINGERS:
            ...
            for i, (base_idx, tip_idx) in enumerate(finger_joints):
                ...
                if base_delta > ctx.contact_thresholds.current_delta_narrow or tip_delta > ctx.contact_thresholds.current_delta_narrow:
                    ...
                    _logger.warning("small_warp: 手指%d接触!", i + 2)
                else:
            ...
            if all(self._finger_stopped):
                _logger.warning("small_warp: 四指闭合完成 → 进入拇指闭合")
                self._phase = _Phase.CLOSE_THUMB
                self._settle_count = 0

            return self._move(list(self._close_angles))

        # === Phase 3: 再闭合拇指（合到四指中间） ===
        if self._phase == _Phase.CLOSE_THUMB:
            ...
            if not self._thumb_stopped:
                ...
                if thumb_delta > ctx.contact_thresholds.current_delta_narrow:
                    self._thumb_stopped = True
                    _logger.warning("small_warp: 拇指接触!")
                else:
            ...
            if self._thumb_stopped:
                _logger.warning("small_warp: 拇指闭合完成 → HOLD")

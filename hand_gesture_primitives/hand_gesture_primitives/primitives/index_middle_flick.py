...
# O20 semantic 20-DOF: 食指中指伸出、其余握拳
POINT_ANGLES = [
    120.0,       # [0]  thumb_base: 收拢
    0.0,         # [1]  index_base: 伸直
    0.0,         # [2]  middle_base: 伸直
    ...
    64.0,        # [16] index_tip: 微弯（摆动中心）
    77.0,        # [17] middle_tip: 微弯（摆动中心）
    255.0,       # [18] ring_tip: 握拳
    255.0,       # [19] pinky_tip: 握拳
]


class IndexMiddleFlick(HandGesturePrimitive):
    ...
    def compute(
        ...
        if t >= self.TRANSITION_DURATION and t < self.FLICK_DURATION:
            ...
            flick_angles[16] = POINT_ANGLES[16] - (self.TIP_SWING_RANGE - 13) * circle_tip_pos
            # 中指摆动
            flick_angles[2] = POINT_ANGLES[2] + self.ROOT_SWING_RANGE * (1 - circle_root_pos)
            flick_angles[17] = POINT_ANGLES[17] - self.TIP_SWING_RANGE * circle_tip_pos

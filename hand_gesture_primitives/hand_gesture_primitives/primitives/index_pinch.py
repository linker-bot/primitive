...
_ref_gesture_params = load_static_gesture_params(
    CANONICAL_SEMANTIC_HAND, "index_pinch")
INDEX_PINCH_ANGLES = list(_ref_gesture_params.target_angles)
...
class IndexPinch(HandGesturePrimitive):
    ...
    def _ensure_engine(self, ctx: PrimitiveContext) -> StaticPoseEngine:
        if self._engine is None or self._hand_type != ctx.hand_type:
            engine = make_static_engine(ctx.hand_type, "index_pinch")
    ...
    def compute(
        ...
        hw = current_monitor_indices(HandConfig(ctx.hand_type), "index_pinch")
        ...
        if stop:
            ...
            _logger.warning(
                "index_pinch: 停指 reason=%s progress=%.0f%% | %s",

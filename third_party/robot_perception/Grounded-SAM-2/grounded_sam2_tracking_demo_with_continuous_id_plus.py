...
Argument parsing
"""
parser = argparse.ArgumentParser(description="Grounded SAM 2 Continuous ID Tracking + Reverse Tracking")
parser.add_argument("--video_path", type=str, required=True, help="Path to input video file (.mp4) or directory of frames")
parser.add_argument("--text_prompt", type=str, required=True, help="Text prompt for detection (e.g. 'car')")
parser.add_argument("--output_path", type=str, default="./outputs", help="Output directory")
parser.add_argument("--step", type=int, default=20, help="Frame step for Grounding DINO detection")
parser.add_argument("--box_threshold", type=float, default=0.25, help="Box threshold for detection")
parser.add_argument("--text_threshold", type=float, default=0.25, help="Text threshold for detection")
parser.add_argument("--frame_rate", type=int, default=15, help="Output video frame rate")
parser.add_argument("--model_id", type=str, default="IDEA-Research/grounding-dino-tiny", help="HuggingFace model ID for Grounding DINO")
...
text = args.text_prompt.lower()
if not text.endswith("."):
    text += "."

# Handle video input: if it's a video file, extract frames to a temp directory
video_input = args.video_path
if os.path.isfile(video_input):
    video_dir = os.path.join(args.output_path, "input_frames")
    CommonUtils.creat_dirs(video_dir)
    ...
    while True:
        ...
        frame_idx += 1
    cap.release()
    print(f"Extracted {frame_idx} frames from video to {video_dir}")
    detected_fps = int(cv2.VideoCapture(video_input).get(cv2.CAP_PROP_FPS))
    if detected_fps > 0 and args.frame_rate == 15:
        args.frame_rate = detected_fps
else:
...
create_video_from_images(result_dir, output_video_path, frame_rate=args.frame_rate)